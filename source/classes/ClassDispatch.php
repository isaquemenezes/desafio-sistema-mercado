...
class ClassDispatch{
    ...
    private function verificaParametros()
    {
        if($this->cont == 1 && empty($this->url[0])){
            $this->page=DIRREQ.'source/views/index.php';
        }else{
            $this->verificaDir();
        }
    }
    ...
    private function verificaFile()
    {
        ...
        if(file_exists($dirAbs.$this->file.'.php'))
        {
            ...
            $this->page = DIRREQ.'source/views/404.php';
        }
    }
    ...
    {
        $page_initial_views = $this->page;
        return $page_initial_views;
    }
}